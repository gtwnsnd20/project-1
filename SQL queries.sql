...
CREATE TABLE thread (thread_id serial PRIMARY KEY, cat_id INT REFERENCES category (cat_id),subject VARCHAR(120),thread_description VARCHAR(120),user_id INT,create_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP(2), CONSTRAINT fk_user_id FOREIGN KEY(user_id) REFERENCES users(user_id) ON DELETE SET NULL);
